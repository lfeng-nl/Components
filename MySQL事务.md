...
- read -view: 不同时刻启动的事务会有不同的`read-view`.  事物内部只能看到当前`Read-View`的值. 所以可以实现`可重读`级别.
    - 决定了事务执行期间"能看到什么数据"
- 回滚日志(`undo log`): 记录着回滚到某个时刻需要执行的操作. 也就是通过回滚日志, 可以得到前一个状态的值.
- 每个事务, 会有一个唯一的事务ID`transaction id`. 由小到大分配.
    - ![mvcc](./image/mvcc2.jpg)
    - 事务更新数据, 会生成一个新的数据版本, 对应一个`row trx_id`, 就是对应的事务id.
    - `V1, V2, V3`每次使用时需要根据当前版本和`undo log`计算出来.
- **"快照"**
    - 通过数据版本的`row trx_id`和当前事务id大小就可以确定当前事务可以看到的信息.
...
> 参考: 极客时间 <<MySQL实战45讲>> 20, 21,30小节.
>
> 根据加锁范围, MySQL锁大致可以分成全局锁, 表级锁, 行锁等三种.
>
> 加锁规则(可重读级别):
>
> 原则1: 加锁的基本单位是`next-key lock`, 前开后闭.
>
> 原则2: 查找过程中访问到的**对象**才会加锁.
>
...
> 一个事务在前后两次查询同一范围的时候, 后一次查询到了前一次查询没有看到的行.
>
> 间隙锁可以随着数据范围的变动而变动
