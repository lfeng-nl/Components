...
>
> 大多数 MySQL 索引(PRIMARY KEY, UNIQUE, INDEX, FULLTEXT) 存储在 B 树, 空间数据类型的索引使用 R 树.
>
> 参考: **<<高性能 MySQL>> 第五章**

### 1.索引类型

- B-Tree 索引: 使用 B-Tree(B+Tree)数据结构来存储数据;
...
- 单列索引:
  ...
  - 索引前缀: 只用指定字段的指定前缀长度(**最长 1000 个字节**)作为索引, `CREATE TABLE xxx( xxx, INDEX(col(10)))`,;
  - 全文索引: 查找的是文本中的关键词, 而不是直接比较索引中的值, `MyISAM`和`InnoDB`支持;
  - 空间索引: 基于 R-Tree, `MyISAM`支持, 可以用于地理数据存储;
...
- 多列索引
  ...
  - 当服务器需要对多个索引做联合操作时(条件有多个 OR), 通常需要耗费大量的 CPU 和内存;
...
> 事务的 ACID:
>
> ​ A 原子性, 事务要么操作全部执行, 要么回滚;
>
> ​ C:一致性, 数据必须保证从一种状态转换为另一种一致性状态;
>
> ​ I: 隔离性, 事务未执行完成, 通常会保证其他 Session 无法看到结果;
>
> ​ D:持久性, 事务一旦 commit, 数据就会保留;

- 原理: InnoDB 具有自动提交特性, 普通 MySQL 执行语句后, 当前的数据提交操作均可被其他客户端可见, 而事务是暂时关闭"自动提交"机制, 需要 commit 提交持久化数据;
- 术语
  - 事务(transaction): 一组 SQL 语句;
  - 回退(rollback): 撤销指定 SQL 语句的过程;
  - 提交(commit): 将未存储的 SQL 语句结果写入数据表;
...
- 控制事务:
  ...
  - `COMMIT`: 提交;
- 更改默认提交行为
  - `SET autocommit=0`: 不自动提交(默认自动提交, 每执行一条 SQL, 立刻生效); **标志针对每个连接而不是服务器**;

### 1.事务隔离级别 isolation levels

- **`READ-UNCOMMITTED` 读未提交**: 一个事务可以读到另一个事务未提交的修改(基本没用);

- **`READ-COMMITTED` 读提交**

  - 如果事务写操作提交(commit), 其他事务中可以读到更新后的信息;
...
- **`SERIALIZABLE` 串行化**: 事务串行执行;
  - 如果前面事务未提交, 后续事务写阻塞, 如果有修改未提交, 读也会阻塞;
- 脏读, 幻读, 不可重读:
  - **脏读**: 查询到其他事务未提交的数据;
  - **幻读**: 在同一个事务中, 使用相同的查询语句, 会多或者少一些数据;
  - **不可重读**: 在同一个事务中, 使用相同的查询语句, 同一资源莫名的改变了;
