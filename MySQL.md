...
> 协调多个进程/线程并发访问某一资源的机制.
>
> MySQL中, 不同的存储引擎支持的锁机制不同.
...
- 锁的颗粒度(InnoDB)
  ...
  - 除单个SQL组成的事务, 锁是逐步获取的, 这就导致死锁发生的可能.

### 1.InnoDB中的锁 [参考](https://dev.mysql.com/doc/refman/5.7/en/innodb-locking.html)
