...
> **REPEATABLE-READ 可重复读**: MySQL 的默认隔离级别(MySQL 上存在丢失更新问题)
...
- 一致性非锁定读取(**快照读**):
  ...
  - 在 RR 级别: 发出普通的查询时, 为事务提供了一个时间点, 根据时间点来查询数据库(查时间点前的数据快照).
  - 在 RC 级别: 普通查询, 读取记录的最新版本(已提交的最新版本).
