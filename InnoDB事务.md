...
> 隔离级别和锁是两个不同概念. 隔离级别有多重实现方式, 最为常见的方式就是加锁, 此外还有时间戳, 多版本控制等无锁并发控制技术;
>
> **LBCC (Lock-Based Concurrent Control) 基于锁的并发控制**: 实现了并发读, 对于并发读写,效率低;
>
> **MVCC(Multi-Version Concurrent Control) 多版本并发控制**:  实现读写并行;
...
- `SELECT ...;`: 读快照, 不加锁.
- `SELECT ... LOCK IN SHARE MODE;`: 读当前, 加 S 锁.
- `SELECT ... FOR UPDATE`: 读当前, 加 X 锁.
- DML(数据操作, 增删改), 加 X 锁.
...
- 一致性非锁定读取(**快照读**):
  ...
  - 使用普通的`select`语句.
  - 使用 MVCC 在某个时间点查询显示数据库的快照.
  - **RR 级别**: 发出普通的查询时, 为事务提供了一个时间点, 根据时间点来查询数据库(查时间点前的数据快照).
  - **RC 级别**: 普通查询, 读取记录的最新版本(已提交的最新版本).
- 加锁读(**当前读**)

  - 读取的不再是快照版本, 而是数据库最新版本, 并会对数据加锁.
  - `SELECT ... LOCK IN SHARE MODE;`: 读取并加 S 锁.
  - `SELECT ... FOR UPDATE;`: 读取并加 X 锁.
- 事务中的`INSERT,UPDATE,DELETE`会加 X 锁.

  - RC 级别: 只加记录锁.
  - RR 级别: 加间隙锁, 解决幻读问题.
- 实现细节:
    - Innodb记录了三个隐藏字段: 事务id, 回滚指针, 隐藏id
    - 实现依赖: undo log (一行原始数据和多个版本镜像数据)和 read view(用于判断当前版本的可见性);
...
> 如果要保证已提交的事务持久存储, 就需要将数据写入(`fsync`)磁盘, 如果直接将数据写入磁盘, 由于数据本身是由一定特殊结构组成, 这就造成写入数据慢等问题.
>
> 可以将写(`fsync`)数据转换为写顺序化的日志, 以提高写入效率.

### 1.Redo log

- 通过`redo log`记录事务提交的修改, 只将`redo log`刷新到磁盘, 系统启动后, 通过`redo log`一样可以恢复数据, 这样就保证了持久性.
